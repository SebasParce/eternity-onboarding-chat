...
TikTok LIVE, programa oficial Backstage). Este repo cubre la **Fase 1**: el
módulo de CHAT (WhatsApp). El módulo de e-learning es la Fase 2 y comparte la
misma base de datos de estado del creador.
...
cp .env.example .env   # dejar WHATSAPP_ADAPTER=mock
npm run dev
...
- Fase 2: plataforma e-learning (perfil, módulos, validación de setup) — consume la misma tabla `creators`.
