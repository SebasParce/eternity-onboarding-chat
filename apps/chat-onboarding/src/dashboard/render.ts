...
import type { Creator, CreatorStage, InteractionLogEntry, ModuleKey, ModuleProgress, SetupValidation } from "../types/domain.js";
...
// --- Academia de creadores (Fase 2) — resumen de progreso para el dashboard ---

const MODULE_LABELS: Record<ModuleKey, string> = {
  perfil: "Mi Perfil",
  bienvenida: "Bienvenida a la agencia",
  monetizacion: "Monetización",
  setup_espacio: "Set-up de espacio",
  reglas: "Reglas de transmisión",
  validacion_setup: "Validación de setup",
...
interface ModuleSummary {
  label: string;
  pillHtml: string;
  /** true si necesita atención de un mentor (vencido, o validación pendiente). */
  needsAttention: boolean;
}

/**
 * De todas las filas de module_progress de un creador, encuentra "en qué va"
 * — el primer módulo (en MODULE_ORDER) que no está completado. Si todos lo
 * están, ya se graduó.
 */
function summarizeModuleProgress(rows: ModuleProgress[]): ModuleSummary | null {
  if (rows.length === 0) return null;
  const byKey = new Map(rows.map((r) => [r.module_key, r]));
  const current = MODULE_ORDER.map((key) => byKey.get(key)).find((r) => r && r.estado !== "completado");

  if (!current) {
    return {
      label: "🎓 Graduado",
      pillHtml: `<span style="font-size:12px;font-weight:600;color:#008069;background:#d9fdd3;padding:3px 10px;border-radius:999px;">🎓 Graduado</span>`,
      needsAttention: false,
    };
  }

  const label = MODULE_LABELS[current.module_key];
  if (current.estado === "vencido_esperando_extension") {
    return {
      label: `⏱️ ${label} (vencido)`,
      pillHtml: `<span style="font-size:12px;font-weight:600;color:#b91c1c;background:#fde2e1;padding:3px 10px;border-radius:999px;">⏱️ ${esc(label)} — vencido</span>`,
      needsAttention: true,
    };
  }
  if (current.estado === "en_progreso" && current.module_key === "validacion_setup") {
    return {
      label: `👀 ${label} (en revisión)`,
      pillHtml: `<span style="font-size:12px;font-weight:600;color:#b45309;background:#fef3e2;padding:3px 10px;border-radius:999px;">👀 ${esc(label)} — en revisión</span>`,
      needsAttention: true,
    };
  }
  return {
    label,
    pillHtml: `<span style="font-size:12px;font-weight:600;color:#7c3aed;background:#f1e9fe;padding:3px 10px;border-radius:999px;">📚 ${esc(label)}</span>`,
    needsAttention: false,
  };
}
...
export function renderCreatorsList(creators: Creator[], moduleByCreator: Map<string, ModuleProgress[]>): string {
  const now = Date.now();
  const rows = creators
    .map((c) => {
      const isNew = now - new Date(c.creado_at).getTime() < 24 * 60 * 60 * 1000;
      const moduleSummary = summarizeModuleProgress(moduleByCreator.get(c.id) ?? []);
      return `<tr style="border-bottom:1px solid ${WA.headerBorder};">
        ...
        <td style="padding:12px 16px;">${moduleSummary ? moduleSummary.pillHtml : `<span style="font-size:12px;color:${WA.textMuted};">—</span>`}</td>
        <td style="padding:12px 16px;">
          ${
            c.bot_pausado
                ...
                <table style="width:100%;border-collapse:collapse;">
                  <thead>
                    <tr style="border-bottom:1px solid ${WA.headerBorder};">
                      ...
                      <th style="text-align:left;padding:11px 16px;font-size:11px;font-weight:700;letter-spacing:.05em;text-transform:uppercase;color:${WA.textMuted};">Módulo (Academia)</th>
...
export function renderRegistros(creators: Creator[], moduleByCreator: Map<string, ModuleProgress[]>): string {
...
export function renderConversation(
  ...
  setupValidations: SetupValidation[]
): string {
  ...
  // --- Academia de creadores (Fase 2): progreso módulo por módulo, más
  // acciones que solo puede hacer un mentor (aprobar/rechazar setup,
  // extender un módulo vencido) — el resto de las transiciones las controla
  // apps/elearning directamente con el creador.
  const moduleByKey = new Map(moduleProgress.map((m) => [m.module_key, m]));
  const pendingValidation = setupValidations.find((v) => v.estado === "pendiente");

  const moduleRowHtml = (key: ModuleKey) => {
    const row = moduleByKey.get(key);
    ...
    const extendButton =
      estado === "vencido_esperando_extension"
        ? `<form method="POST" action="/dashboard/creators/${esc(creator.id)}/modules/${esc(key)}/extend" style="margin-top:4px;">
             <button type="submit" style="font-size:11px;background:${WA.accent};color:#fff;border:none;padding:4px 10px;border-radius:6px;">Extender 48h</button>
           </form>`
        : "";
    return `<div style="display:flex;justify-content:space-between;align-items:flex-start;padding:6px 0;font-size:12.5px;">
      <span>${icon} ${esc(MODULE_LABELS[key])}</span>
      <div style="text-align:right;">
        ${row?.expires_at && (estado === "disponible" || estado === "en_progreso") ? `<div style="color:${WA.textMuted};font-size:11px;">hasta ${esc(fullDate(row.expires_at))}</div>` : ""}
        ${extendButton}
      </div>
    </div>`;
  };

  const validationReviewHtml = pendingValidation
    ? `<div style="margin-top:8px;padding:10px;background:#fef3e2;border-radius:8px;">
        <div style="font-size:12px;color:#92400e;margin-bottom:6px;">
          📷 Envío del ${esc(fullDate(pendingValidation.creado_at))} esperando revisión
          ${
            pendingValidation.brightness_score !== null && pendingValidation.brightness_score >= 0
              ? ` · brillo ${Math.round(pendingValidation.brightness_score)}/255`
              : ""
          }
        </div>
        <div style="display:flex;gap:8px;">
          <form method="POST" action="/dashboard/creators/${esc(creator.id)}/validations/${esc(pendingValidation.id)}/approve">
            <button type="submit" style="font-size:12px;background:${WA.accent};color:#fff;border:none;padding:6px 12px;border-radius:6px;font-weight:600;">✓ Aprobar</button>
          </form>
          <form method="POST" action="/dashboard/creators/${esc(creator.id)}/validations/${esc(pendingValidation.id)}/reject">
            <button type="submit" style="font-size:12px;background:transparent;color:#b91c1c;border:1px solid #f5b5b5;padding:6px 12px;border-radius:6px;">✕ Rechazar</button>
          </form>
        </div>
      </div>`
    : "";
  ...
  const academyHtml =
    moduleProgress.length > 0
      ? `<div style="padding:9px 0;border-bottom:1px solid ${WA.headerBorder};">
          <div style="font-size:11px;color:${WA.textMuted};text-transform:uppercase;letter-spacing:.04em;margin-bottom:4px;">Academia de creadores</div>
          ${MODULE_ORDER.map(moduleRowHtml).join("")}
          ${validationReviewHtml}
        </div>`
      : "";
